...
namespace nap
{
    ...
    PointCloudShader::PointCloudShader(Core& core) : Shader(core),
        mRenderService(core.getService<RenderService>()), mRealSenseService(core.getService<RealSenseService>()) { }
    ...
    bool PointCloudShader::init(utility::ErrorState& errorState)
    {
        ...
        const std::string vertex_shader_path = mRealSenseService->getModule().findAsset(relative_path);
        if (!errorState.check(!vertex_shader_path.empty(), "%s: Unable to find %s vertex shader %s", mRealSenseService->getModule().getName().c_str(), shader::pointcloud, vertex_shader_path.c_str()))
            return false;

        relative_path = utility::joinPath({ "shaders", utility::appendFileExtension(shader::pointcloud, "frag") });
        const std::string fragment_shader_path = mRealSenseService->getModule().findAsset(relative_path);
        if (!errorState.check(!vertex_shader_path.empty(), "%s: Unable to find %s fragment shader %s", mRealSenseService->getModule().getName().c_str(), shader::pointcloud, fragment_shader_path.c_str()))
            return false;
        ...
        // Compile shader
        return this->load(std::string(shader::pointcloud), vert_source.data(), vert_source.size(), frag_source.data(), frag_source.size(), errorState);
    }
}
